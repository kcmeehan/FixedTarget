...
TH1D *GetYieldHistogram(TH3D *yieldHisto3D, Double_t rapidity, Double_t mTm0, TString detector){
  ...
  //Since the Global yIndex value starts at 0 and the first rapidity bin starts at 1,
  //we need to shift by one for the naming scheme
  Int_t yIndex = rapidityBin - 1;
  Int_t mTm0Index = mTm0Bin - 1;
  
  //Create the 1D Histogram 
  TH1D *zProj = yieldHisto3D->ProjectionZ(Form("%s_%d_%d",yieldHisto3D->GetName(),yIndex,mTm0Index),
  ...
  zProj->SetTitle(Form("%s, y= [%g,%g], m_{T}-m_{0}= [%g,%g] (GeV/c^{2})",yieldHisto3D->GetTitle(),
...
//______________________________________________________________________________________________________
void LoadYieldHistograms(TH3D *yieldHisto3D, std::vector<TH1D *> *zHistos,
			 TString detector, Int_t rapidityIndex){

  //Load the Yield histograms for yIndex=rapidityIndex into the zHistos vector
  for (Int_t mTm0Index=0; mTm0Index<nmTm0Bins; mTm0Index++){

    zHistos->at(mTm0Index) =
      GetYieldHistogram(yieldHisto3D,GetRapidityRangeCenter(rapidityIndex),
...
void DefineParticlePars(ParticlePars *p, TString name){
  ...
  for (Int_t yIndex=0; yIndex<nRapidityBins; yIndex++){
    ...
    p->widthConf[yIndex] = new TGraphErrors();
    p->widthConf[yIndex]->SetName(Form("widthConfGraph_%s_%02d",name.Data(),yIndex));
    p->widthConf[yIndex]->SetTitle(Form("%s Width Confidence Interval  %02d",name.Data(),yIndex));
    p->widthConf[yIndex]->SetMarkerStyle(20);
