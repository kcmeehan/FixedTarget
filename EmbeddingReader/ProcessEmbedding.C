...
#include "/project/projectdirs/starprod/rnc/meehan/Embedding/AuAu_4_5GeV_2015/EmbeddingForThesis/globalDefinitions.h"
#include "readAuAuFile.C"

//Struct of holding Event Info                      
struct EventType {
...
void ProcessEmbedding(const char* MINIMCLIST, TString OUTPUTFILE, TString CENTDB,
		      Int_t NCENTBINS, Double_t ENERGY, Int_t PIDINTEREST, Int_t EVENTNAME){
  ...
  Int_t nEmbTracks(0), nMatchTracks(0), nSplitTracks(0), refMult(0), originMult(0), centrality(0), centralMult(0);
  ...
  fileChain->SetBranchAddress("mNUncorrectedPrimaries",&refMult);
  fileChain->SetBranchAddress("mCentralMult",&centralMult);
  fileChain->SetBranchAddress("mOriginMult",&originMult);
  fileChain->SetBranchAddress("mCentrality",&centrality);
  ...
  nMatchTrackHisto = new TH1F("nMatchTrackHisto",Form("nMatchedTracks %s %s;nMatchTracks",pidSymbol,energy),
			      20,0,20);
  refMultHisto     = new TH1F("refMultHisto",Form("RefMult %s %s;refMult",pidSymbol,energy),
			      400,0,400);
  originMultHisto  = new TH1F("originMultHisto",Form("N Daughters %s %s;originMult (number of daughters)",
						     pidSymbol,energy),
			      400,0,400);
  centralityHisto  = new TH1F("centralityHisto",Form("Centrality %s %s;centrality",pidSymbol,energy),
			      400,0,400);
  centralMultHisto = new TH1F("centralMultHisto",Form("Multiplicity %s %s;centralMult",pidSymbol,energy),
			      400,0,400);
  embZVertexHisto  = new TH1F("embZVertexHisto",Form("V_{z} %s %s;z Vertex (cm)",pidSymbol,energy),
			      100,event.minVz-10,event.maxVz+10);
  embEventPhiHisto = new TH1F("embEventPhiHisto",Form("Event #phi %s %s;Event #phi",pidSymbol,energy),
			      100,-3.2,3.2);
  ...
  for (int centIndex=0; centIndex<nCentBins; centIndex++){
    embTrackHisto[centIndex] = new TH2F(Form("embTrackHisto_Cent%d",centIndex),
					Form("EmbTracks Rapididty Vs. m_{T}-m_{%s} %s; y_{%s}^{Emb};(m_{T}-m_{%s})^{Emb} (GeV)",
  ...
  for (Long64_t chainIndex=0; chainIndex<nEntries; chainIndex++){
    ...
    if (centralityBin < 0) continue; //exclude non-index-0 vertices (-999), pileup vertices (-2), and peripheral > 30% cenral bins (-1)

    //Fill The Event Level Histograms                                                                   
    nEmbTrackHisto->Fill(nEmbTracks);
    nMatchTrackHisto->Fill(nMatchTracks);
    refMultHisto->Fill(refMult);
    originMultHisto->Fill(originMult);
    centralityHisto->Fill(centrality);
    centralMultHisto->Fill(centralMult);
    embZVertexHisto->Fill(embVertexZ);
    embEventPhiHisto->Fill(TMath::ATan2(embVertexY,embVertexX));
...
void SetEventType(EventType *eventType, Int_t eventName){
  ...
  else if (eventName == kAuAuFixedTarget2015){
    eventType->minVz = 210;
    eventType->maxVz = 212;
    eventType->minVr = 0;
    eventType->maxVr = 100;
