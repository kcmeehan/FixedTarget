...
void DirectedFlowKbg(char *filename = "test.root", int rebin = 1, double EventPlaneResolution = 0.404, char *histogram = "V1vsM", int linear=0, float sigma = 0.5)
{
...
//  const int nYBins = 7;
//  double y_bins[nYBins] = {-1.4,-1.2,-1.0,-0.8,-0.6,-0.4,-0.2};
  const int nYBins = 10;
  double y_bins[nYBins] = {-1.36,-1.2,-1.1,-1.0,-0.9,-0.8,-0.7,-0.6,-0.5,-0.4};
//  const int nYBins = 14;
//  double y_bins[nYBins] = {-1.36,-1.24,-1.18,-1.12,-1.06,-1,-0.9,-0.8,-0.7,-0.6,-0.5,-0.4,-0.3,-0.2};
//  const int nYBins = 36;
//  double y_bins[nYBins] = {-1.6,-1.56,-1.52,-1.48,-1.46,-1.44,-1.42,-1.4,-1.38,-1.36,-1.34,-1.32,-1.3,-1.28,-1.26,-1.24,-1.22,-1.2,-1.18,-1.16,-1.14,-1.12,-1.1,-1.08,-1.06,-1.04,-1.02,-1.0,-0.9,-0.8,-0.7,-0.6,-0.5,-0.4,-0.3,-0.2};
...
//    cout<<fYield->Integral(y_bins_cms[i],y_bins_cms[i+1])/(y_bins_cms[i+1]-y_bins_cms[i])<<endl;
  ...
  TFitResultPtr fitPointer = gFlow4->Fit("g","0VS","",0,upLimFit);
...
//  ftodrawreflected->Draw("same");
  ...
  TFile *output = new TFile("BackgroundFlowK0.root","recreate");
  gFlow4->SetName("BgK0Flow");
  gFlow4->Write();
  gFlowReflected->SetName("BgK0FlowReflectedToDraw");
  gFlowReflected->Write();
  g->SetName("fitFunctionOfBgK0");
  g->Write();
