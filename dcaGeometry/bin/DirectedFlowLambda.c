...
void DirectedFlowLambda(char *filename = "test.root", int rebin = 1, double EventPlaneResolution = 0.404, char *histogram = "V1vsM", int linear=0, float sigma = 0.5)
{
...
//  inputfile->Close();
  TFile *bgFile = new TFile("BackgroundFlowLambda.root");
  TGraphErrors *gBg = (TGraphErrors*)bgFile->Get("BgLambdaFlow");
  TF1 *fBg = (TF1*)bgFile->Get("fitFunctionOfBgLambda");
  fBg->SetName("fBg");
  hUnlikeSign->Sumw2();
...
//  const int nYBins = 12;
//  double y_bins[nYBins] = {-1.5,-1.3,-1.2,-1.1,-1.0,-0.9,-0.8,-0.7,-0.6,-0.5,-0.4,-0.2};
...
//    cout<<fYield->Integral(y_bins_cms[i],y_bins_cms[i+1])/(y_bins_cms[i+1]-y_bins_cms[i])<<endl;
  ...
  TH2D *hTemp = new TH2D("Temp","",10,-1.5,1.5,1000,-0.4,0.4);
  ...
  TFitResultPtr fitPointer = gFlow4->Fit("g","0VS","",0,2);
...
//  ftodrawreflected->Draw("same");
  ...
  TLine *lx0 = new TLine(0,-0.4,0,0.4);
  lx0->SetLineStyle(2);
  ...
  TFile *output = new TFile("FlowLambda.root","recreate");
  gFlow4->SetName("LambdaFlow");
  gFlow4->Write();
  gFlowReflected->SetName("LambdaFlowReflectedToDraw");
  gFlowReflected->Write();
  g->SetName("fitFunctionOfLambda");
  g->Write();
