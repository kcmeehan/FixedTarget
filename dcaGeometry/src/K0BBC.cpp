...
TLorentzVector LV_K0s(0,0,0,0);
const float MASS_PI = 0.13957; // in GeV
const float MASS_MUON = 0.105658; // in GeV
const float MASS_K = 0.49368; // in GeV
const float MASS_PHI = 1.019445; // in GeV
const float MASS_D0 = 1.86484; // in GeV
const float MASS_Dstar = 2.0102762; // in GeV
const float MASS_P = 0.93827; // in GeV
const float TRIG_MUON_PT = 2.5; // in GeV
...
int BufferEvent_nPions[BUFFER_SIZE+10*BUFFER_SIZE];
int BufferEvent_Centrality[BUFFER_SIZE+10*BUFFER_SIZE];
int BufEv_Charge[BUFFER_SIZE+10*BUFFER_SIZE][500];
StPhysicalHelixD BufEv_Helix[BUFFER_SIZE+10*BUFFER_SIZE][500];
StThreeVectorD BufEv_MomVector[BUFFER_SIZE+10*BUFFER_SIZE][500];

void makeMixedPairs(int bufferPointer, int nPi, Particle *pPi, StThreeVectorD primVtxPos, Float_t MagnetField, int centrality);
void copyCurrentToBuffer(int bufferPointer, int nPi, Particle *pPi, int centrality);
...
void PicoDst::Loop( int nEvents = -1)
{
  ...
  hUnlikeSignVsY = new TH2D("UnlikeSignVsY","",100,-2.,0.,400,0.3,0.7);
  hRotatedVsY = new TH2D("RotatedVsY","",100,-2.,0.,400,0.3,0.7);
  hMixedVsY = new TH2D("MixedVsY","",100,-2.,0.,400,0.3,0.7);
  ...
  hV1vsM = new TH3D("V1vsM","",200,0.4,0.6,220,-1.1,1.1,100,-2,0);
  hV1tpc = new TH3D("V1tpc","",200,0.4,0.6,220,-1.1,1.1,100,-2,0);
  hV1vsMPt = new TH3D("PtV1vsM","",200,0.4,0.6,220,-1.1,1.1,10,0.2,2.2);
  hV1tpcPt = new TH3D("PtV1tpc","",200,0.4,0.6,220,-1.1,1.1,10,0.2,2.2);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) { // event loop
    ...
    if(jentry%100==0) cout << " Procesing event " << jentry << " ... " << endl;
...
//    if (Event_mPrimaryVertex_mX2[0] < -2.3 || Event_mPrimaryVertex_mX2[0] > -1.3) continue;
    ...
    //    double mCentWeight = -999;
    //    double grefMultCorr = 0;
    //    StRefMultCorr* grefmultCorrUtil = CentralityMaker::instance()->getgRefMultCorr() ;
    //    grefmultCorrUtil->setVzForWeight(6, -30.0, 30.0);
    //    grefmultCorrUtil->readScaleForWeight("/Users/DavidTlustyRice/Muons/DiMuons2014/dcaGeometry/include/StRefMultCorr/macros/weight_grefmult_vpd30_vpd5_Run14.txt");
    //    if(grefmultCorrUtil)
    //    {
    //      grefmultCorrUtil->init(Event_mRunId[0]);
    //      grefmultCorrUtil->initEvent(Event_mGRefMult[0],Event_mPrimaryVertex_mX3[0],Event_mZDCx[0]);
    //      mCentrality = grefmultCorrUtil->getCentralityBin16();
    //      mCentWeight = grefmultCorrUtil->getWeight();
    //      grefMultCorr = grefmultCorrUtil->getRefMultCorr();
    
    //      cout<<mCentrality<<"   "<<mCentWeight<<"   "<<grefMultCorr<<endl;
    //    }
    //    hCentrality->Fill(mCentrality);
    //    hCentWeight->Fill(mCentrality,mCentWeight);
    
    
    
    //******************************* Track Selection ****************************************************
    nPions = 0; nPrimTracks = 0;
    int nK0s = 0;
    ...
    for(int i = 0; i<Tracks_; i++){
      ...
      hEta->Fill(TMath::Abs(p.pseudoRapidity()));
      ...
      if (gDca>8) continue;
      if(pmag <= 0) continue;
      if(p.perp() <= 0.05) continue;
      //      hDedx->Fill(pmag,TMath::Log(Tracks_mDedx[i]*0.001)*charge);
      hNsigmaPion->Fill(pmag*charge,Tracks_mNSigmaPion[i]);
      hMom->Fill(pmag);
      
      //        if(gDca > 1) continue;
      
      double OneOverBetaTofPredictedForPions = TMath::Sqrt(1+MASS_PI*MASS_PI/pmag/pmag);
      double OneOverBetaTof = 20000./(double)BTofPidTraits_mBTofBeta[Tracks_mBTofPidTraitsIndex[i]];
      if (gDca > 1 /*&& gDca > 6./TMath::Power(p.mag()+1,4)*/ && Tracks_mNSigmaPion[i] > -300 && Tracks_mNSigmaPion[i] < 300) {
        if ( ((OneOverBetaTof-OneOverBetaTofPredictedForPions) < -0.012 || (OneOverBetaTof-OneOverBetaTofPredictedForPions) > 0.016) && Tracks_mBTofPidTraitsIndex[i]>=0) continue;
        pPion[nPions].Id = Tracks_mId[i];
        pPion[nPions].Charge = charge;
        pPion[nPions].P = p;
        pPion[nPions].helix = trackHelix;
        pPion[nPions].GDCA = gDca;
        pPion[nPions].nSigmaPion = Tracks_mNSigmaPion[i];
        //          pPion[nPions].TofOneOverBeta = OneOverBetaTof;
        nPions++;
      }
    }
    ...
    //**************************** Create Unlike-sign Pairs for K0s Analysis ***********************************
    // pNeg is negative
    // pPos is positive
    
    for (int i=0; i<nPions; i++) {
      StPhysicalHelixD part1Helix = pPion[i].helix;
      TLorentzVector lvI, lvJ;
      StThreeVectorD p1 = pPion[i].P;
      lvI.SetPtEtaPhiM(p1.perp(),p1.pseudoRapidity(),p1.phi(),MASS_PI);
      for (int j=i; j<nPions; j++) {
        if (pPion[i].Id == pPion[j].Id) continue;
        if (pPion[i].Charge + pPion[j].Charge == 0) {
          StThreeVectorD p2 = pPion[j].P;
          lvJ.SetPtEtaPhiM(p2.perp(),p2.pseudoRapidity(),p2.phi(),MASS_PI);
          TLorentzVector EstLV_K0s = lvI + lvJ;
          if (EstLV_K0s.M()<0.4 || EstLV_K0s.M()>0.6) continue; // very loose cut on ivariant mass to speed up analysis
          ...
          if (decLenVec.mag() < 3) continue; // cut on path length
          if (dcaVector.mag() > 1) continue; // cut on track mutual dca
          StThreeVectorD pNegMomAtDca = part1Helix.momentumAt(pathLenghts.first, Event_mBField[0] * kilogauss);
          StThreeVectorD pPosMomAtDca = part2Helix.momentumAt(pathLenghts.second, Event_mBField[0] * kilogauss);
          StThreeVectorD K0sMomAtDCA = pNegMomAtDca + pPosMomAtDca;
          lvI.SetPtEtaPhiM(pNegMomAtDca.perp(),pNegMomAtDca.pseudoRapidity(),pNegMomAtDca.phi(),MASS_PI);
          lvJ.SetPtEtaPhiM(pPosMomAtDca.perp(),pPosMomAtDca.pseudoRapidity(),pPosMomAtDca.phi(),MASS_PI);
          TLorentzVector LV_K0s = lvI + lvJ;
          double K0pT = LV_K0s.Pt();
          double pointingAngle = K0sMomAtDCA.angle(decLenVec);
          if (TMath::Abs(pointingAngle) > 0.106+0.056-0.1123*K0pT+0.025*K0pT*K0pT) continue; // cut on pointing angle
//          if (TMath::Abs(pointingAngle) > 0.1) continue; // cut on pointing angle
          double V0DCAtoPrimVtx  = TMath::Abs(decLenVec.mag()*TMath::Sin(pointingAngle));
          if (V0DCAtoPrimVtx > 1) continue;
          hUnlikeSignVsY->Fill(LV_K0s.Rapidity(),LV_K0s.M());
          //          if (LV_K0s.M() > 0.490 && LV_K0s.M() < 0.508 && LV_K0s.Eta() < -1.2){
          if (LV_K0s.Eta() < 0){
            vis[pPion[i].Id] = true;
            vis[pPion[j].Id] = true;
            KY[nK0s] = LV_K0s.Rapidity();
            KEta[nK0s] = LV_K0s.PseudoRapidity();
            KPt[nK0s] = LV_K0s.Pt();
            KPhi[nK0s] = LV_K0s.Phi();
            KM[nK0s] = LV_K0s.M();
            nK0s++;
            if (LV_K0s.M() > 0.490 && LV_K0s.M() < 0.508) hPtY->Fill(LV_K0s.Rapidity(),K0pT);
    ...
    //**************************** Create Rotated-momentum Pairs for K0s Analysis ***********************************
    // pNeg is negative
    // pPos is positive
...
//      StThreeVectorD p1 = pPion[i].P;
      ...
      for (int j=i; j<nPions; j++) {
        if (pPion[i].Id == pPion[j].Id) continue;
        if (pPion[i].Charge + pPion[j].Charge != 0) continue;
        StThreeVectorD p2 = pPion[j].P;
        lvJ.SetPtEtaPhiM(p2.perp(),p2.pseudoRapidity(),p2.phi(),MASS_PI);
        TLorentzVector EstLV_K0s = lvI + lvJ;
        if (EstLV_K0s.M()<0.4 || EstLV_K0s.M()>0.6) continue; // very loose cut on ivariant mass to speed up analysis
...
//        if (TMath::Abs(pointingAngle) > 0.1) continue; // cut on pointing angle
        double V0DCAtoPrimVtx  = TMath::Abs(decLenVec.mag()*TMath::Sin(pointingAngle));
        if (V0DCAtoPrimVtx > 1) continue;
        hRotatedVsY->Fill(LV_K0s.Rapidity(),LV_K0s.M());
      } // loop j through nPions
    } // loop i trough nPions
    ...
    double mQExCorrected = mQEx - 2.921519e-04;
    ...
    double cos12 = TMath::Cos(12*psi_e);    psi_e_s  = psi_e + 2*(0.000561787)*sin - 2*(0.00400706)*cos + (-0.0588185)*sin2 - (-8.60469e-05)*cos2 + 0.5*(0.0156012)*sin4 - 0.5*(-0.000444193)*cos4 + 1./3.*(-0.0117927)*sin6 - 1./3.*(-0.00020104)*cos6 + 1./4.*(0.00215868)*sin8 - 1./4.*(0.00077722)*cos8 + 1./5.*(0.00172388)*sin10 - 1./5.*(0.00113641)*cos10 + 1./6.*(-0.00133822)*sin12 - 1./6.*(-0.000967033)*cos12;
    ...
    for(int i = 0; i<Tracks_; i++){
      ...
      //      TRandom3 *gRandom3 = new TRandom3(0);
    ...
    for (int iK0s=0; iK0s<nK0s; iK0s++){
      ...
      double QxCorrected1 = qx1 - 1.733275e-03;
      double QyCorrected1 = qy1 - 3.473946e-02;
      hQxQy1->Fill(qx1,qy1);
      ...
      double PsiCorrected1 = psi1 + 2*(0.000315464)*Sin1 - 2*(0.0117923)*Cos1 + (0.00356834)*Sin21 - (0.00286694)*Cos21 + 0.5*(-0.00192985)*Sin41 - 0.5*(-0.00121338)*Cos41;
      ...
      double QxCorrected2 = qx2 + 6.975870e-04;
      double QyCorrected2 = qy2 - 5.342696e-03;
      hQxQy2->Fill(qx2,qy2);
      ...
      double PsiCorrected2 = psi2 + 2*(-0.000221224)*Sin2 - 2*(-0.000996093)*Cos2 + (-0.01233)*Sin22 - (0.0110343)*Cos22 + 0.5*(-0.00219309)*Sin42 - 0.5*(0.00281479)*Cos42;
      ...
      hV1vsM->Fill(KM[iK0s],TMath::Cos(KPhi[iK0s]-psi_e_s),KY[iK0s]);
      hV1tpc->Fill(KM[iK0s],TMath::Cos(KPhi[iK0s]-PsiCorrected1),KY[iK0s]);
      if(KY[iK0s]>-0.8 && KY[iK0s]<0){
        hV1vsMPt->Fill(KM[iK0s],TMath::Cos(KPhi[iK0s]-psi_e_s),KPt[iK0s]);
        hV1tpcPt->Fill(KM[iK0s],TMath::Cos(KPhi[iK0s]-PsiCorrected1),KPt[iK0s]);
      }
    }
...
//    BufferPointer = (int)mCentrality*62 + (int)((PsiCorrectedA + 0.5*TMath::Pi())*20);
    hBufferPointer->Fill(BufferPointer);
    if(BufferPointer>=BUFFER_SIZE||BufferPointer<0) continue;
//    makeMixedPairs(BufferPointer, nPions, pPion, priVtx, Event_mBField[0], mCentrality);
//    copyCurrentToBuffer(BufferPointer, nPions, pPion, mCentrality);
...
void makeMixedPairs(int bufferPointer, int nPi, Particle *pPi, StThreeVectorD primVtxPos, Float_t MagnetField, int centrality){
  ...
  for(int k = 0; k < BufferEvent_NEvents[bufferPointer]; k++){
    for(int i = 0; i < nPi; i++){
      ...
      lvI.SetPtEtaPhiM(p1.perp(),p1.pseudoRapidity(),p1.phi(),MASS_PI);
      for(int j=i; j<BufferEvent_nPions[k*BUFFER_SIZE+bufferPointer]; j++){
        if (pPi[i].Charge + BufEv_Charge[k*BUFFER_SIZE+bufferPointer][j] == 0) {
          lvJ.SetPtEtaPhiM(BufEv_MomVector[k*BUFFER_SIZE+bufferPointer][j].perp(),BufEv_MomVector[k*BUFFER_SIZE+bufferPointer][j].pseudoRapidity(),BufEv_MomVector[k*BUFFER_SIZE+bufferPointer][j].phi(),MASS_PI);
          TLorentzVector EstLV_K0s = lvI + lvJ;
          //if (EstLV_K0s.M()<0.35 || EstLV_K0s.M()>0.7) continue;
          StPhysicalHelixD HelixJ = BufEv_Helix[k*BUFFER_SIZE+bufferPointer][j];
          ...
          if (TMath::Abs(pointingAngle) > 0.106+0.056-0.1123*K0pT+0.025*K0pT*K0pT) continue; // cut on pointing angle
          double V0DCAtoPrimVtx  = TMath::Abs(decLenVec.mag()*TMath::Sin(pointingAngle));
          if (V0DCAtoPrimVtx > 0.8) continue;
//          hMixed->Fill(K0pT,LV_K0s.M(),BufferEvent_Centrality[k*BUFFER_SIZE+bufferPointer]);
          hMixedVsY->Fill(LV_K0s.Rapidity(),LV_K0s.M());
...
void copyCurrentToBuffer(int bufferPointer, int nPi, Particle *pPi, int centrality){
  ...
  if(BufferEvent_NEvents[bufferPointer]>=10) {
  ...
  if(BufferEvent_Full[bufferPointer]) { // full - random rewrite one
    do {
      ...
      eventPointer = (int)(10.*(1.0 - rrr));
    } while(eventPointer<0||eventPointer>=10);
  ...
  for(int i=0; i<nPi; i++){
    BufEv_Charge[bufferPointer+BUFFER_SIZE*eventPointer][i] = pPi[i].Charge;
    BufEv_Helix[bufferPointer+BUFFER_SIZE*eventPointer][i] = pPi[i].helix;
    BufEv_MomVector[bufferPointer+BUFFER_SIZE*eventPointer][i] = pPi[i].P;
  }
  if(BufferEvent_NEvents[bufferPointer]<10) BufferEvent_NEvents[bufferPointer]+=1;
}
...
//  fChain->SetBranchStatus("Event.mRanking", 1);
