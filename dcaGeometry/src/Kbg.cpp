...
void PicoDst::Loop( int nEvents = -1)
{
  ...
  hNlambdaPerEvent = new TH1I("NlambdaPerEvent","",100,0,100);
  ...
  hProtonEta = new TH1D("ProtonEta","",1000,-10,10);
...
//  hV1vsM = new TH3D("V1vsM","",10,arrayOfLowEdgesX,150,arrayOfLowEdgesV1,40,arrayOfLowEdgesZ);
  hV1vsM = new TH3D("V1vsM","",200,0.4,0.6,300,-1.5,1.5,100,-2,0);
  hV1tpc = new TH3D("V1tpc","",200,0.4,0.6,300,-1.5,1.5,100,-2,0);
...
//    if (Event_mPrimaryVertex_mX2[0] < -2.3 || Event_mPrimaryVertex_mX2[0] > -1.3) continue;
    ...
    for(int i = 0; i<Tracks_; i++){
      ...
      double pmag = primMom.magnitude();
      if(pmag>0) {
        nPrimTracks++;
        hMom->Fill(pmag);
        hEta->Fill(primMom.pseudoRapidity());
        ...
        if (Tracks_mNSigmaPion[i] > -200 && Tracks_mNSigmaPion[i] < 200){
          if((OneOverBetaTof-OneOverBetaTofPredictedForPions) > -0.012 && (OneOverBetaTof-OneOverBetaTofPredictedForPions) < 0.016 && pmag < 3.) {
            pPion[nPions].Id = Tracks_mId[i];
            pPion[nPions].Charge = charge;
            pPion[nPions].P = primMom;
            nPions++;
          }else if(pmag < 1.2){
//            pPion[nPions].Id = Tracks_mId[i];
//            pPion[nPions].Charge = charge;
...
//            nPions++;
    ...
    for (int i=0; i<nPions; i++) {
      ...
      for (int j=0; j<nPions; j++) {
        ...
        if (EstLV_Lambda.M()<0.4 || EstLV_Lambda.M()>0.6) continue; // very loose cut on ivariant mass to speed up analysis
...
//        if (pProton[i].GDCA < 0.4 || pPion[j].GDCA < 0.8) continue; // get rid of primary tracks
        hUnlikeSignVsY->Fill(EstLV_Lambda.Rapidity(),EstLV_Lambda.M());
//        if (LV_Lambda.M() > 1.085 && LV_Lambda.M() < 1.19 && LV_Lambda.Eta() < -1.2){
//        if (LV_Lambda.M() > 1.113 && LV_Lambda.M() < 1.119 /*&& LV_Lambda.Eta() < -1.2*/){
        if (EstLV_Lambda.Eta() < 0){
          LY[nLambda] = EstLV_Lambda.Rapidity();
          LEta[nLambda] = EstLV_Lambda.PseudoRapidity();
          LPt[nLambda] = EstLV_Lambda.Pt();
          LPhi[nLambda] = EstLV_Lambda.Phi();
          LM[nLambda] = EstLV_Lambda.M();
          LProtonId[nLambda] = pProton[i].Id;
          hPhi->Fill(EstLV_Lambda.Phi());
          nLambda++;
        }
...
//    cout<<"end of filling"<<endl;
    ...
    for(int iTile = 0; iTile < 16; iTile++) {
      if(Event_mBbcAdcEast[0][iTile]>10 && Event_mBbcAdcEast[0][iTile]<4000){
        ...
        //          eWgt += nHitE[iTile];
        //        }else{
        eXsum += TMath::Cos(BBC_GetPhi(0,iTile,jentry*(iTile+1)))*nHitE[iTile];
        eYsum += TMath::Sin(BBC_GetPhi(0,iTile,jentry*(iTile+1)))*nHitE[iTile];
...
//      StThreeVectorD GlobMom = trackHelix.momentum( Event_mBField[0] * kilogauss  );
      ...
      if(p > 0.3 && eta > -1 && eta < 0 && /*gDca < 3 &&*/ charge > 0){
        //        if(pt > 2) pt = 2;
        if(Tracks_mNSigmaProton[i] > -200 && Tracks_mNSigmaProton[i] < 200){
...
//    double QyCorrectedA = (qyA - 3.024766e-02)*NtrkA;
    double QxCorrectedA = qxA - 4.390028e-03;
    double QyCorrectedA = qyA - 4.936262e-02;
    hQxQyA->Fill(qxA,qyA);
    ...
    double PsiCorrectedA = psiA + 2*(0.00171195)*SinA - 2*(0.0150135)*CosA + (0.00759872)*Sin2A - (0.00515145)*Cos2A + 0.5*(0.000343612)*Sin4A - 0.5*(-4.78431e-05)*Cos4A;
...
//    double QyCorrectedB = (qyB - 2.263891e-03)*NtrkB;
    double QxCorrectedB = qxB + 4.139620e-03;
    double QyCorrectedB = qyB - 7.336551e-03;
    hQxQyB->Fill(qxB,qyB);
    ...
    double PsiCorrectedB = psiB + 2*(0.000519294)*SinB - 2*(-0.0016399)*CosB + (-0.0126564)*Sin2B - (0.00983453)*Cos2B + 0.5*(0.000897834)*Sin4B - 0.5*(-0.000441895)*Cos4B;
...
//        if(TMath::Abs(Tracks_mNHitsFit[i]/(double)Tracks_mNHitsMax[i]) < 0.52) continue;
//        if(Tracks_mNHitsDedx[0] <= 0) continue;
//        if(TMath::Abs(Tracks_mNHitsFit[i]) < 25) continue;
//        float charge = Tracks_mNHitsFit[i] > 0 ? 1 : -1;
//        StThreeVectorD primMom = StThreeVectorD(Tracks_mPMomentum_mX1[i],Tracks_mPMomentum_mX2[i],Tracks_mPMomentum_mX3[i]);
//        double pt = primMom.perp();
//        double p = primMom.mag();
...
//        }
//        double SigmaDeuteron = TMath::Log(Tracks_mDedx[i]*0.001) - gDeuterons->Eval(p);
//        if(p > 0.3 && eta > -0.7 && eta < 0 && charge > 0){
//          //        if(pt > 2) pt = 2;
...
//          }
//        }
//        if(p > 0.3 && eta > -1.1 && eta < -0.7 && charge > 0){
//          //        if(pt > 2) pt = 2;
//          if(Tracks_mNSigmaProton[i] > -300 && Tracks_mNSigmaProton[i] < 300){
//            double E = TMath::Sqrt(MASS_P*MASS_P+p*p);
//            double y = 0.5*TMath::Log((E+Tracks_mPMomentum_mX3[i])/(E-Tracks_mPMomentum_mX3[i]));
...
//          }
//        }
//        if(p > 0.3 && eta > -1.5 && eta < -1.1 && charge > 0){
//          //        if(pt > 2) pt = 2;
//          if(Tracks_mNSigmaProton[i] > -300 && Tracks_mNSigmaProton[i] < 300){
//            double E = TMath::Sqrt(MASS_P*MASS_P+p*p);
//            double y = 0.5*TMath::Log((E+Tracks_mPMomentum_mX3[i])/(E-Tracks_mPMomentum_mX3[i]));
...
//      if(Ntrk1 < 4 || Ntrk2 < 4 || Ntrk3 < 4) continue;
...
//      double QxCorrected1 = qx1 - 2.314979e-03;
//      double QyCorrected1 = qy1 - 4.211970e-02;
//      hQxQy1->Fill(qx1,qy1);
...
//      double PsiCorrected1 = psi1 + 2*(0.00101676)*Sin1 - 2*(0.0130729)*Cos1 + (0.000656622)*Sin21 - (0.00141046)*Cos21 + 0.5*(0.000925789)*Sin41 - 0.5*(-0.00216454)*Cos41;
//      
...
//      double QxCorrected2 = qx2 - 7.084937e-03;
//      double QyCorrected2 = qy2 - 1.196768e-02;
//      hQxQy2->Fill(qx2,qy2);
...
//      double PsiCorrected2 = psi2 + 2*(0.000820108)*Sin2 - 2*(0.000665674)*Cos2 + (-0.00646313)*Sin22 - (0.00760463)*Cos22 + 0.5*(-0.000169749)*Sin42 - 0.5*(0.00258418)*Cos42;
//
...
//      hBBCvsTpc1eventPlane->Fill(psi_e_s,PsiCorrected1);
      ...
      hV1tpc->Fill(LM[iLambda],TMath::Cos(LPhi[iLambda]-PsiCorrectedA),LY[iLambda]);
