...
//  hV1vsM = new TH3D("V1vsM","",10,arrayOfLowEdgesX,150,arrayOfLowEdgesV1,40,arrayOfLowEdgesZ);
  hV1vsM = new TH3D("V1vsM","",120,1.08,1.2,300,-1.5,1.5,100,-2,0);
  hV1tpc = new TH3D("V1tpc","",120,1.08,1.2,300,-1.5,1.5,100,-2,0);
...
//    if (Event_mPrimaryVertex_mX2[0] < -2.3 || Event_mPrimaryVertex_mX2[0] > -1.3) continue;
    ...
    for(int i = 0; i<Tracks_; i++){
      ...
      if(primMom.mag()>0) {
        nPrimTracks++;
        hMom->Fill(primMom.mag());
        hEta->Fill(primMom.pseudoRapidity());
        if (Tracks_mNSigmaPion[i] > -200 && Tracks_mNSigmaPion[i] < 200 && charge < 0) {
          pPion[nPions].Id = Tracks_mId[i];
          pPion[nPions].Charge = charge;
          pPion[nPions].P = primMom;
          nPions++;
        }
        if (primMom.perp() > 0.2 && Tracks_mNSigmaProton[i] > -200 && Tracks_mNSigmaProton[i] < 200 && charge > 0) {
          pProton[nProtons].Id = Tracks_mId[i];
          pProton[nProtons].Charge = charge;
          pProton[nProtons].P = primMom;
          nProtons++;
        }
      }
    }
...
//    cout<<"Q-vectors: "<<mQEx<<"   "<<mQEy<<endl;
    ...
    double mQExCorrected = mQEx - 1.083343e-04;
    double mQEyCorrected = mQEy + 4.047196e-03;
...
//    psi_e -= TMath::Pi();
    ...
    psi_e_s  = psi_e + 2*(0.000626539)*sin - 2*(0.00320486)*cos + (0.0217035)*sin2 - (-0.000517205)*cos2 + 0.5*(-0.010968)*sin4 - 0.5*(3.00131e-05)*cos4 + 1./4*(-0.00257218)*sin6 - 1./4*(0.000870863)*cos6  + 1./8*(-0.0013673)*sin8 - 1./8*(-0.000848787)*cos8;
...
//    double QyCorrectedA = (qyA - 3.024766e-02)*NtrkA;
    double QxCorrectedA = qxA - 5.114302e-03;
    double QyCorrectedA = qyA - 3.826910e-02;
    hQxQyA->Fill(qxA,qyA);
    ...
    double PsiCorrectedA = psiA + 2*(0.00185653)*SinA - 2*(0.012689)*CosA + (0.00213879)*Sin2A - (0.000417636)*Cos2A + 0.5*(0.000243742)*Sin4A - 0.5*(-0.00023207)*Cos4A;
...
//    double QyCorrectedB = (qyB - 2.263891e-03)*NtrkB;
    double QxCorrectedB = qxB + 8.980481e-04;
    double QyCorrectedB = qyB - 3.463330e-03;
    hQxQyB->Fill(qxB,qyB);
    ...
    double PsiCorrectedB = psiB + 2*(0.000148022)*SinB - 2*(-0.00136342)*CosB + (-0.0174215)*Sin2B - (0.0112866)*Cos2B + 0.5*(-0.00141233)*Sin4B - 0.5*(-0.000722356)*Cos4B;
