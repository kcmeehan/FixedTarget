...
void StHbtCorrFctnDirectYlm::AddRealPair(const StHbtPair* aPair, double weight)
{
	...
	for (int ilm = 0; ilm < GetMaxJM(); ilm++) {
		...
		if ( std::isinf(real(ylmbuffer[ilm]) * weight) ) return;
		if ( std::isinf(imag(ylmbuffer[ilm]) * weight) ) return;
	}
	if ( nqbin < binctn->GetNbinsX() )
		for (int ilmzero = 0; ilmzero < GetMaxJM(); ilmzero++)
			for (int ilmprim = 0; ilmprim < GetMaxJM(); ilmprim++) {
				...
				if ( std::isinf(real(ylmbuffer[ilmzero]) * real(ylmbuffer[ilmprim]) * weight * weight) ) return;
				if ( std::isinf(real(ylmbuffer[ilmzero]) * -imag(ylmbuffer[ilmprim]) * weight * weight) ) return;
				if ( std::isinf(imag(ylmbuffer[ilmzero]) * real(ylmbuffer[ilmprim]) * weight * weight) ) return;
				if ( std::isinf(imag(ylmbuffer[ilmzero]) * -imag(ylmbuffer[ilmprim]) * weight * weight) ) return;
			}
	...
	if (mDoEMCIC) {
		...
		if ( !( std::isnan(p1p2t) || std::isnan(p1p2z) || std::isnan(e1e2) || std::isnan(e1pluse2) ||
				std::isinf(p1p2t) || std::isinf(p1p2z) || std::isinf(e1e2) || std::isinf(e1pluse2) ) )
		{
			for (int ilm = 0; ilm < GetMaxJM(); ilm++) {
