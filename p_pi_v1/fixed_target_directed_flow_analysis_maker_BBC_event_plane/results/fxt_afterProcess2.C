...
void fxt_afterProcess2() {
    TFile *inputFile = new TFile("total.root","read");
    //TFile *inputFile = new TFile("test_0.root","read");
    ...
    TFile *outputFile = new TFile("result02.root","recreate");
    // flow histograms
    int Ncentralities = 7;
    ...
    // set EP indicator
    //Bool_t chooseBBCeastEP = kTRUE, chooseTPCEP = kFALSE;
    Bool_t chooseBBCeastEP = kFALSE, chooseTPCEP = kTRUE;
            ...
            for(int momentumBin=momentumBottomBin;momentumBin<=momentumTopBin;momentumBin++) {
                if(profile3D_deuteron_v1->GetBinError(icent,momentumBin,bin) > 0.0) {
                    ...
                    if(profile3D_deuteron_v1->GetBinEntries(currentBin) >= 5) {
                        double binContent = profile3D_deuteron_v1->GetBinContent(currentBin);
                        double binEntry = profile3D_deuteron_v1->GetBinEntries(currentBin);
                        double binError = profile3D_deuteron_v1->GetBinError(currentBin);
                        double ss = binError*TMath::Sqrt((double)binEntry);
                        content += binContent*binEntry;
                        contentS+= (ss*ss+binContent*binContent)*binEntry;
                        entry   += (int)binEntry;
