...
//parameterize Boltzmann function to supply the weight
static TF1 boltz("boltz","x*[0]*sqrt(x*x+1.115684*1.115684)*exp(-sqrt(x*x+1.115684*1.115684)/[1])",0,10.0);
const int kParms=2;
Double_t fitparms[kCentBin*kParms]={0.000143739,0.166894,0.00271168,0.189314,0.0393703,0.202126,0.535079,0.208564,6.76524,0.214754,89.4392,0.216197};

//parameterize levy function to supply the weight
//TF1 levy("levy","x*(([0]*([1]-1)*([1]-2))/([1]*[2]*([1]*[2]+1.115684*([1]-2))))/(1+((TMath::Sqrt((x*x)+(1.115684*1.115684))-1.115684)/([1]*[2])))^[1]",0.,10.0);
//const int kParms2=3;
//Double_t fitparms2[kCentBin*kParms2]={1.02084e-07,34.9996,0.159115,4.55194e-06,35,0.178925,0.000115197,35,0.185109,0.00147135,45,0.205502,0.0278512,45,0.20217,0.381043,45,0.202733,5.35698,45,0.20089};
static TF1 levy("boltz2","x*[0]*sqrt(x*x+1.115684*1.115684)*exp(-sqrt(x*x+1.115684*1.115684)/[1])",0,10.0);
const int kParms2=2;
Double_t fitparms2[kCentBin*kParms2]={8.87989e-05,0.184912,0.00245464,0.205143,0.0482163,0.2137,0.615949,0.222023,9.38849,0.224951,131.13,0.225456};

void cuts_la_cut0(int nlist, int block, int expswitch, int fswitch){
...
//the following will be loaded in .rootlogon.C in ACLIC mode
   ...
   TChain * t = NULL;
   if(!expswitch){
	t = ChainThem("lafp_100_124.pico.list","McV0PicoDst",nlist,block);
   }
   else{
	t = ChainThem("laexp_100_119.pico.list","McV0PicoDst",nlist,block);
   }
   ...
   ifstream parms(Form("input/auau5_la_fit_cut0_%s.txt",funcname[fswitch]));
   if(parms){
	cout<<"READING function parameters from data file "<<Form("input/auau5_la_fit_cut0_%s.txt",funcname[fswitch])<<endl;
   ...
   TH1F * hmIMcheck = new TH1F("hmIMcheck","check inv mass", 440, pdgmass-masswidth, pdgmass+masswidth);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	for(Int_t i=0;i<dst.nv0;i++){
	   ...
	   */
	   ...
	   if(dst.mcv0pt[ind]<ptbd[ptbin]*0.75)continue; //a simple way to remove the outliers, the number should be tuned for each kind of particle
   ...
   TString ofilename;
   if(!expswitch) ofilename=Form("output/weight_la_cut0_%s_fp.txt",funcname[fswitch]);
   else ofilename=Form("output/weight_la_cut0_%s_exp.txt",funcname[fswitch]);
   ...
   //output the details
   TString ofile2name;
   if(!expswitch) ofile2name=Form("output/weight_la_cut0_%s_fp_appd.txt",funcname[fswitch]);
   else ofile2name=Form("output/weight_la_cut0_%s_exp_appd.txt",funcname[fswitch]);
...
Double_t getWeight(Int_t cent, Float_t pt, int expswitch){
   ...
   boltz.SetParameters(&fitparms[cent*kParms]);
   
   if (funcswitch) wgt = boltz.Eval(pt);

   levy.SetParameters(&fitparms2[cent*kParms2]);

   if (!funcswitch) wgt = levy.Eval(pt);
