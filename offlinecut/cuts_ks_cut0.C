...
void cuts_ks_cut0(int nlist, int block){
...
//the following will be loaded in .rootlogon.C in ACLIC mode
   ...
   TChain * tfg = ChainThem("finished_ks.list","V0PicoDst",nlist,block);
   if(!tfg){ cout<<"ERROR: no files are added to the chain!"<<endl; return; }
   TChain * tbg = ChainThem("finished_ksrot.list","V0PicoDst",nlist,block);
   if(!tbg){ cout<<"ERROR: no files are added to the chain!"<<endl; return; }

   //histograms
   TString Dir("output/");
   TString Name("auau5.mb.ks."); 
   ...
   TH2D * hNRefMultNV0  = new TH2D( "RefMultV0", "Reference Multiplicity of selected events vs nV0", 600, 0.0, 600.0, 1000,0,5000.0 ) ;

   TH1F * hNV0  = new TH1F( "NV0", "number of v0 candidates in selected events", 1000, 0.0, 5000.0 ) ;
   TH1F * hNV0s  = new TH1F( "NV0s", "number of selected v0 candidates in selected events", 1000, 0.0, 5000.0 ) ;
   TH2D * hNV0NV0s  = new TH2D( "NV0NV0s", "nV0 vs nV0s", 1000, 0.0, 5000.0, 1000,0,5000.0 ) ;
   ...
   //book histograms
   TH1F * hmInvMass = new TH1F("hmInvMass","Invariant mass", 552,pdgmass-masswidth,pdgmass+masswidth);
   ...
   for(Int_t iIM=0; iIM<kCentBin; iIM++){
	...
	hmIMbgCent[iIM] = new TH1F(hNamebg, hTitle, 552, pdgmass-masswidth, pdgmass+masswidth);
	hmIMCent[iIM]->Sumw2();
	...
	for(Int_t jIM=0; jIM<kPtBin; jIM++){
	   ...
	   hmIMbg[iIM][jIM] = new TH1F(hNamebg, hTitle, 552, pdgmass-masswidth, pdgmass+masswidth);
	   hmIM[iIM][jIM]->Sumw2();
   ...
   for(Int_t ind = 0; ind < 2; ind++){
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   if(ind==0)hNV0s->Fill(nv0select);
	   if(ind==0)hNV0NV0s->Fill(dst.nv0,nv0select);
	}
	delete t;
   }
