...
void reconstruct_mcv0_k0s(TString InputFileList, Int_t Nlist, Int_t Block = 50, Int_t nEvents = 0, TString OutputDir = "output/");

void reconstruct_mcv0_k0s(TString InputFileList, Int_t Nlist, Int_t Block, Int_t nEvents, TString OutputDir) 
{
  // Load libraries
  ...
  TString mMoreTagsFile = mEventFile;
  int fileBeginIndex = mMoreTagsFile.Index("st_",0);
  mMoreTagsFile.Remove(0,fileBeginIndex);
  const int indx1 = mMoreTagsFile.Index(".geant",0);
  ...
  mMoreTagsFile.Prepend("./moretags/");
  cout<<"The moretags file is: "<<mMoreTagsFile<<endl;
  ...
  parameterDB->setReqCommonHitsTpc(5); // Require 5 hits in common for
  ...
  mcsig -> setMoreTagsFileName(mMoreTagsFile) ; // Need a special tags files to retrieve refmult from data mudst
  ...
  if ( nEvents == 0 )  nEvents = 10000 ;       // Take all events in nFiles if nEvents = 0
