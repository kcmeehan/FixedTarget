...
{ // Initialize and/or zero all public/private data members here.
  ...
  mSecondaryV0 = false; //v0 is from primary vertex by default

  mDcaAlgoLong = true;

  moretags_input = NULL  ;                    // Zero the Pointer to moretags file
  ...
  mMoreTagsFileName = "" ;               // more tags file
...
void StMcV0Maker::initConst(){
   ...
   if(mV0Type == kLambda || mV0Type == kAntiLambda){
	...
	if(mSecondaryV0) mGeantIDV0  = (mV0Type == kLambda)? 98:97;
	else mGeantIDV0  = (mV0Type == kLambda)? 10018:10026;
...
void StMcV0Maker::initParam(){
   ...
   //cutTriggerIdEq  = 280001; //dau run8
   cutTriggerIdEq  = 1; //FXT
...
void StMcV0Maker::initTree()
{
   ...
   mV0Tree->Branch("nrefmultembed",&mV0Dst.nrefmultembed,"nrefmultembed/I");
   ...
   mV0Tree->Branch("v0ximcid",mV0Dst.v0ximcid,"v0ximcid[nv0]/I");
   ...
   mV0Tree->Branch("dau1ncommhits",mV0Dst.dau1ncommhits,"dau1ncommhits[nv0]/I");
   ...
   mV0Tree->Branch("dau2ncommhits",mV0Dst.dau2ncommhits,"dau2ncommhits[nv0]/I");
...
Int_t StMcV0Maker::Init( )
{
  ...
  // open moretags input file
  if(mMoreTagsFileName == "") { 
     //CAUTION: ALWAYS USE { } HERE!!! LOG_XXX is a if()xxx macro!!!
     LOG_WARN << "StMcV0Maker: The moretags input file is not specified! will not record refmult from MuDst!" <<endm;
  }
  else {
     moretags_input = new TFile( mMoreTagsFileName ) ;  
  }
  if(moretags_input) mMoreTagsTree = (TTree *) moretags_input->Get("MoreTags");
...
{ // Do each event
  ...
  */
  ...
  // cut on vertexZ
  if (fabs(primaryVertex->position().z()) >= 212 ||  fabs(primaryVertex->position().z()) <= 210 ) return kStOK ;
  ...
  mV0Dst.nrefmultembed = refmultiplicity;
  ...
  if(mMoreTagsTree){
     Int_t nFound =  (Int_t) mMoreTagsTree->Draw("nRefMult:nTofMatch",
	     Form("RunId==%i&&EvtId==%i", mV0Dst.runnumber, mV0Dst.evtnumber), "goff");
     if(nFound!=1){
	  cout<<"run #: "<<mV0Dst.runnumber<<" evt #: "<<mV0Dst.evtnumber<<" not found in "<<mMoreTagsFileName<<endl;
	  mV0Dst.nrefmult = -1;
     }
     else {
	  mV0Dst.nrefmult = mMoreTagsTree->GetV1()[0];
	  mV0Dst.ntofmatched= mMoreTagsTree->GetV2()[0];
     }
  }
  else{
     mV0Dst.nrefmult = -1;
  ...
  mV0Dst.bbccirate = event->runInfo()->bbcCoincidenceRate();
  mV0Dst.zdccirate = event->runInfo()->zdcCoincidenceRate();
  ...
  for(unsigned int nMc=0; nMc < McTrack.size(); nMc++){
     ...
     if(McTrack[nMc]->parent()){
	  cout<<"Parent: "<< McTrack[nMc]->parent()->geantId() <<" Key: "<<McTrack[nMc]->parent()->key()<<endl;
  ...
  for(unsigned int i=0;i<mDauVec1.size();i++){
     ...
     for(unsigned int j=0;j<mDauVec2.size();j++){
	  ...
	  int count = 0;
	  int nCommonHits1=0,nCommonHits2=0;
	  StTrackPairInfo *candTrackPair1 = 0, *candTrackPair2 = 0;
	  for (StMcVertexIterator mcVertexIt = mcVertices.begin();
	     ...
	     for (mcTrackMapIter mctrkIter1 =  mcTrackBounds1.first; mctrkIter1!= mcTrackBounds1.second; mctrkIter1++) {
		  ...
		  for(mcTrackMapIter mctrkIter2 =  mcTrackBounds2.first; mctrkIter2!= mcTrackBounds2.second; mctrkIter2++) {
		     ...
		     count++;
		     VerSave=mcVertexIt;
		     candTrackPair1 = (*mctrkIter1).second;
		     candTrackPair2 = (*mctrkIter2).second;
		  }//match pion
	     }//match proton
	  }//mcVertex
	  ...
	  nCommonHits1 = candTrackPair1->commonTpcHits()%100;
	  nCommonHits2 = candTrackPair2->commonTpcHits()%100;
	  
	  //record the MC key of V0 parent track (say Xi)
        int xikey = -1;
        const StMcTrack* v0track = (*VerSave)->parent();
        const StMcTrack* xitrack = v0track->parent();
        if (xitrack) cout<<"xxx: "<<xitrack->geantId()<<" "<<xitrack->key()<<endl;
        if (xitrack) xikey = xitrack->key();
	  ...
	  */
	  ...
	  double theta = atan2(sinth, rdotp/xv0toPV.mag()/pv0.mag()); //theta range: from 0 to pi
        ...
        Float_t mdEdx1, mdEdx2;
        Int_t mNHitsDedx1, mNHitsDedx2;
        StDedxPidTraits* pid1 = findDedxPidTraits(track1);
        mdEdx1  = (pid1) ? pid1->mean() : -999;
        mNHitsDedx1 = (pid1) ? pid1->numberOfPoints() : 0;
        StDedxPidTraits* pid2 = findDedxPidTraits(track2);
        mdEdx2  = (pid2) ? pid2->mean() : -999;
        mNHitsDedx2 = (pid2) ? pid2->numberOfPoints() : 0;


	  mV0Dst.v0mcid[nV0] = (*VerSave)->parent()->key();
	  mV0Dst.v0ximcid[nV0] = xikey;
	  ...
	  mV0Dst.dau1nhitsposs[nV0]= track1->numberOfPossiblePoints(kTpcId);
	  mV0Dst.dau1nhitsdedx[nV0]= mNHitsDedx1;
	  mV0Dst.dau1dedx[nV0] = mdEdx1;
	  ...
	  mV0Dst.dau1ncommhits[nV0]  = nCommonHits1;
	  ...
	  mV0Dst.dau2nhits[nV0]= track2->detectorInfo()->numberOfPoints();
        mV0Dst.dau2nhitsposs[nV0]= track2->numberOfPossiblePoints(kTpcId);
        mV0Dst.dau2nhitsdedx[nV0]= mNHitsDedx2;
	  mV0Dst.dau2dedx[nV0] = mdEdx2;
	  ...
	  mV0Dst.dau2ncommhits[nV0]  = nCommonHits2;
...
StDedxPidTraits* StMcV0Maker::findDedxPidTraits(const StTrack* track)
{
...
{ // Do once at the end the analysis
  ...
  if(moretags_input!=NULL) moretags_input -> Close() ;   // Write all histograms to disk 
