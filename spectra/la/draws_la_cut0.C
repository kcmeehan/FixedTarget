...

{
   ...
   TString ifile = "output/auau"+Ecm+".mb.la."+cuttag+".histo.root";
   TFile ihm(ifile);
   ...
   c1->Divide(Nx,Ny);
   Int_t Ncells = Nx*Ny;
   TString pdfname = "auau"+Ecm+"_la_invmass_"+cuttag+".pdf";
   ...
   for(Int_t iPt=0; iPt<kPtBin; iPt++){
	for(Int_t iIM=0; iIM<kCentBin; iIM++){
	   ...
	   //draw the histogram
	   TString htitle(Form("#Lambda, Au+Au %s GeV, %s, p_{T} %s GeV/c", Ecm.Data(), CentName[iIM], PtName[iPt]));
	   ...
	   fsinggausbg->SetParameters(hmCurrent->GetEntries()/3,pdgmass,0.001,hmCurrent->GetEntries()/3,-10,30,0,0);
	   fsinggausbg->SetParLimits(1,pdgmass-0.005,pdgmass+0.005);
	   fsinggausbg->SetParLimits(2,0.001,0.012);
	   ...
	   bool kFit;
	   if(hmCurrent->GetEntries()>550) kFit = true;
	   else kFit = false;
	   ...
	   fdoubgausbg->SetParLimits(2,0.0005,fsinggausbg->GetParameter(2));
	   fdoubgausbg->SetParLimits(4,fsinggausbg->GetParameter(2),fsinggausbg->GetParameter(2)*2.0);
   ...
   TString oname = "auau"+Ecm+"_la_yield_"+cuttag+".txt";
   ...
   TString oname = "auau"+Ecm+"_la_invmass_peak_sigma_"+cuttag+".txt";
   ofstream oyield(oname);
   for(int iIM=0;iIM<kCentBin;iIM++)
	for(int iPt=0;iPt<kPtBin;iPt++)
