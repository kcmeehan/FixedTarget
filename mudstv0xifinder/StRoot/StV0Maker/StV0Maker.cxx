...
void StV0Maker::initParam(){
   ...
   cutTriggerIdEq  = 1; //auau7 mb
   //cutTriggerIdEq  = 240025; //pp run9 vpd-mb
   ...
   if(mV0Type == kPhoton){
	...
	cutDca1to2LeEq = 2.0; //0.8
	cutV0MassWidthLeEq = 0.12;
	//cutDauPtArmLeEq = 0.3;
	//cutAbsDausPtShoulderDiffLeEq = 1.2;
	//cutDau1DecAngGr = 0.;
	//cutDau2DecAngGr = 0.;
	cutV0rdotpGr  = -10000.0;
	cutDcaV0Le    = 5.0; //3.5
	cutV0DecLenGrEq =3.0;
   }
   else {
...
void StV0Maker::initTree()
{
   ...
   mV0Tree->Branch("zdccirate",&mV0Dst.zdccirate,"zdccirate/D");
   mV0Tree->Branch("ntofmatched",&mV0Dst.ntofmatched,"ntofmatched/I");
   ...
   mV0Tree->Branch("dau1firstx",mV0Dst.dau1firstx,"dau1firstx[nv0]/F");
   mV0Tree->Branch("dau1firsty",mV0Dst.dau1firsty,"dau1firsty[nv0]/F");
   mV0Tree->Branch("dau1firstz",mV0Dst.dau1firstz,"dau1firstz[nv0]/F");
   mV0Tree->Branch("dau1lastx",mV0Dst.dau1lastx,"dau1lastx[nv0]/F");
   mV0Tree->Branch("dau1lasty",mV0Dst.dau1lasty,"dau1lasty[nv0]/F");
   mV0Tree->Branch("dau1lastz",mV0Dst.dau1lastz,"dau1lastz[nv0]/F");
   ...
   mV0Tree->Branch("dau2firstx",mV0Dst.dau2firstx,"dau2firstx[nv0]/F");
   mV0Tree->Branch("dau2firsty",mV0Dst.dau2firsty,"dau2firsty[nv0]/F");
   mV0Tree->Branch("dau2firstz",mV0Dst.dau2firstz,"dau2firstz[nv0]/F");
   mV0Tree->Branch("dau2lastx",mV0Dst.dau2lastx,"dau2lastx[nv0]/F");
   mV0Tree->Branch("dau2lasty",mV0Dst.dau2lasty,"dau2lasty[nv0]/F");
   mV0Tree->Branch("dau2lastz",mV0Dst.dau2lastz,"dau2lastz[nv0]/F");
...
{ // Do each event
  ...
  // Get 'event' data 
  if (! mMuDstMaker->muDst() ) return kStOK;
  ...
  //if ( !muEvent->triggerIdCollection().nominal().isTrigger(290001) && !muEvent->triggerIdCollection().nominal().isTrigger(290004)) return kStOK ; //run3 dAu
  //if ( !muEvent->triggerIdCollection().nominal().isTrigger(210020) && !muEvent->triggerIdCollection().nominal().isTrigger(210000)) return kStOK ; //run8 dAu
  mV0Dst.trgmode=-1; //dummy for run 9
  /*
  if( muEvent->triggerIdCollection().nominal().isTrigger(290001)) mV0Dst.trgmode = 0; //vpd-zdce
  if( muEvent->triggerIdCollection().nominal().isTrigger(290004)) mV0Dst.trgmode = 1; //zdce
  if( muEvent->triggerIdCollection().nominal().isTrigger(290001) && muEvent->triggerIdCollection().nominal().isTrigger(290004)) mV0Dst.trgmode = 2; //zdce
  */
  ...
  for(unsigned int i=0;i<mMuDstMaker->muDst()->numberOfPrimaryVertices();i++) {
     StMuPrimaryVertex *vtx = mMuDstMaker->muDst()->primaryVertex(i);
     if(!vtx) continue;
     Float_t vz = vtx->position().z();
     if(vz>210. && vz<212.) {
	  mMuDstMaker->muDst()->setVertexIndex(i);
	  mV0Dst.trgmode = i;
	  break;
     }
  }
  ...
  */
  ...
  if ( mMuDstMaker->muDst()->numberOfPrimaryTracks() == mTestNTrack && mEventsProcessed !=0 && mTestVZ !=0 &&  muEvent->primaryVertexPosition().z() == mTestVZ ) {
     LOG_WARN << mEventsProcessed <<" "<<"seems a duplicated event! "<<mTestNTrack<<" "<<mTestVZ<<endm;
     return kStOK ;
  }
  ...
  tofpv = muEvent->primaryVertexPosition();

  // Fill some QA plots
  hVertexZ -> Fill( tofpv.z() ) ; // Make histogram of the vertex Z distribution
  hNRefMult -> Fill( muEvent->refMult() );		

  // cut on vertexZ
  //if (fabs(tofpv.z()) > cutAbsVertexZLeEq ) return kStOK ;
  if (tofpv.z() >= 212. || tofpv.z() <=210.  ) return kStOK ;
  ...
  mV0Dst.nrefmultftpceast = muEvent->refMultFtpcEast();
  ...
  mV0Dst.zdccirate = muEvent->runInfo().zdcCoincidenceRate();

  Int_t nMultiTracks = 0;
  Int_t nTofMatPrTrack = 0;
  TObjArray* prtracks = mMuDstMaker->muDst()->primaryTracks() ;    // Create a TObject array containing the global tracks  
  ...
  while ( ( prtrack = (StMuTrack*)GetPrTracks.Next() ) )             // Main loop for Iterating over tracks
  {
     ...
     //if(prtrack->nHitsFit(kTpcId)*1.0/prtrack->nHitsPoss(kTpcId) <= 0.52) continue;
     if(prtrack->nHitsFit()*1.0/prtrack->nHitsPoss() <= 0.52) continue;
     if(prtrack->nHitsDedx()<=0) continue;
     nMultiTracks++;
  }
  mV0Dst.nrefmult = nMultiTracks;
  mV0Dst.ntofmatched = nTofMatPrTrack;
  ...
  if(mV0Dst.trgmode==0 || mV0Dst.trgmode==2)hSelectNRefMultM1 -> Fill( muEvent->refMult() ); //this is an ESSENTIAL histogram to record the total number of events for certain centrality. always make sure it is filled AFTER event selection!
  if(mV0Dst.trgmode==1 || mV0Dst.trgmode==2)hSelectNRefMultM2 -> Fill( muEvent->refMult() ); //this is an ESSENTIAL histogram to record the total number of events for certain centrality. always make sure it is filled AFTER event selection!
  ...
  while ( ( track = (StMuTrack*)GetTracks.Next() ) )             // Main loop for Iterating over tracks
  {
     ...
     if(flag <=0 || flag >=1000 )continue; //or <=0 ?
     ...
     if(charge == mCharge1 && fabs(nsigma)<cutAbsNSigma1Le){
	  ...
	  //fill the vector
	  StPhysicalHelixD helix = track->helix(); //inner helix. good for dca to PV.
	  double pathlength = helix.pathLength(tofpv, false); // do scan periods. NOTE: the default is false. this is not necessary for tracks with pt>0.15GeV/c
	  StThreeVectorF dca = helix.at(pathlength)-tofpv;
     ...
     if(charge == mCharge2 && fabs(nsigma)<cutAbsNSigma2Le){
	  ...
	  double pathlength = helix.pathLength(tofpv, false); // do scan periods. NOTE: the default is false.
	  StThreeVectorF dca = helix.at(pathlength)-tofpv;
  ...
  for(unsigned int i=0;i<mDauVec1.size();i++){
     ...
     for(unsigned int j=0;j<mDauVec2.size();j++){
	  ...
	  mV0Dst.dau1nhitsposs[nV0]= track1->nHitsPoss(kTpcId);
	  ...
	  mV0Dst.dau1firstx[nV0] = track1->firstPoint().x();
	  mV0Dst.dau1firsty[nV0] = track1->firstPoint().y();
	  mV0Dst.dau1firstz[nV0] = track1->firstPoint().z();
	  mV0Dst.dau1lastx[nV0] = track1->lastPoint().x();
	  mV0Dst.dau1lasty[nV0] = track1->lastPoint().y();
	  mV0Dst.dau1lastz[nV0] = track1->lastPoint().z();
	  ...
	  mV0Dst.dau2nhitsposs[nV0]= track2->nHitsPoss(kTpcId);
	  ...
	  mV0Dst.dau2firstx[nV0] = track2->firstPoint().x();
	  mV0Dst.dau2firsty[nV0] = track2->firstPoint().y();
	  mV0Dst.dau2firstz[nV0] = track2->firstPoint().z();
	  mV0Dst.dau2lastx[nV0] = track2->lastPoint().x();
	  mV0Dst.dau2lasty[nV0] = track2->lastPoint().y();
	  mV0Dst.dau2lastz[nV0] = track2->lastPoint().z();
